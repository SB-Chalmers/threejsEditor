...
import { DEFAULT_FACADE_PARAMETERS, MAX_WWR } from '../services/FacadeGeometry';
...
export const BuildingEditPanel: React.FC<BuildingEditPanelProps> = ({ draft, baseDraft, onChange, onReset, onCommit, onCancel }) => {
  ...
  const setField = (key: keyof BuildingEditDraft, value: unknown) => {
    const nextValue = key === 'window_to_wall_ratio'
      ? Math.max(0, Math.min(MAX_WWR, typeof value === 'number' ? value : Number(value)))
      : value;
    onChange({
      ...draft,
      [key]: nextValue,
    } as BuildingEditDraft);
  };
      ...
      <aside className="model-inspector pointer-events-auto fixed bottom-0 right-0 top-12 z-50 flex w-[304px] flex-col border-l border-slate-200 bg-white shadow-xl">
        ...
        <div className="custom-scrollbar flex-1 overflow-y-auto px-4">
          <Section title="Identity" open={open.identity} onToggle={() => toggle('identity')}>
            <Field label="Name"><input className={inputClass} value={draft.name} onChange={event => setField('name', event.target.value)} /></Field>
            <Field label="Description"><textarea className={`${inputClass} h-16 resize-none py-2`} value={draft.description} onChange={event => setField('description', event.target.value)} /></Field>
          </Section>

          <Section title="Form & Massing" open={open.massing} onToggle={() => toggle('massing')}>
            <Field label={`Floors · ${draft.floors}`}><input aria-label="Floors" type="range" min="1" max="50" value={draft.floors} onChange={event => setField('floors', Number(event.target.value))} className="model-range w-full" /></Field>
            <Field label={`Floor height · ${draft.floorHeight.toFixed(1)} m`}><input aria-label="Floor height" type="range" min="2.5" max="6" step="0.1" value={draft.floorHeight} onChange={event => setField('floorHeight', Number(event.target.value))} className="model-range w-full" /></Field>
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {MUTED_COLORS.map(color => <button key={color.name} type="button" aria-label={color.name} title={color.name} onClick={() => setField('color', color.value)} className={`h-7 rounded-md border-2 ${draft.color === color.value ? 'border-blue-600 ring-2 ring-blue-100' : 'border-white ring-1 ring-slate-200'}`} style={{ backgroundColor: `#${color.value.toString(16).padStart(6, '0')}` }} />)}
              </div>
            </div>
          </Section>

          <Section title="Façade" open={open.facade} onToggle={() => toggle('facade')}>
            <Field label={`Window-to-wall ratio · ${Math.round((draft.window_to_wall_ratio ?? DEFAULT_FACADE_PARAMETERS.wwr) * 100)}%`}><input aria-label="Window-to-wall ratio" type="range" min="0" max="0.95" step="0.01" value={draft.window_to_wall_ratio} onChange={event => setField('window_to_wall_ratio', Number(event.target.value))} className="model-range w-full" /></Field>
            <label className="flex items-center gap-2 text-[11px] text-slate-700"><input type="checkbox" checked={draft.window_overhang ?? false} onChange={event => setField('window_overhang', event.target.checked)} /> Additional window overhang</label>
            <Field label={`Additional depth · ${(draft.window_overhang_depth ?? 0).toFixed(2)} m (total ${((draft.window_overhang ? draft.window_overhang_depth ?? 0 : 0) + DEFAULT_FACADE_PARAMETERS.wallThickness).toFixed(2)} m)`}><input aria-label="Additional overhang depth" type="range" min="0" max="2" step="0.05" value={draft.window_overhang_depth ?? 0} onChange={event => setField('window_overhang_depth', Number(event.target.value))} className="model-range w-full" /></Field>
          ...
          <Section title="Program" open={open.program} onToggle={() => toggle('program')}>
            <Field label="Building program"><select className={inputClass} value={draft.building_program} onChange={event => setField('building_program', event.target.value)}>{['Office', 'Residential', 'Retail', 'School', 'Hospital'].map(value => <option key={value}>{value}</option>)}</select></Field>
            <Field label="Structural system"><select className={inputClass} value={draft.structural_system} onChange={event => setField('structural_system', event.target.value)}>{['Concrete', 'Timber', 'Masonry'].map(value => <option key={value}>{value}</option>)}</select></Field>
          </Section>

          <Section title="Systems" open={open.systems} onToggle={() => toggle('systems')}>
            <Field label="HVAC"><select className={inputClass} value={draft.hvac_system} onChange={event => setField('hvac_system', event.target.value)}>{['Default HVAC', 'VAV', 'CAV', 'Radiant', 'Split System'].map(value => <option key={value}>{value}</option>)}</select></Field>
            <label className="flex items-center gap-2 text-[11px] text-slate-700"><input type="checkbox" checked={draft.natural_ventilation ?? false} onChange={event => setField('natural_ventilation', event.target.checked)} /> Natural ventilation</label>
          </Section>
        </div>
